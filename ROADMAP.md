...
| 2026-08-31 | K166 ingest: 3/3 arXiv API false positives → overflow + thin GEO steal + CCC | pyEDITH HWO calculator→overflow only (MIT repo astro OOD no clone); rank-two permanents→overflow only (math.CO); PULSAR pooled late-interaction visual RAG→thin GEO steal (adaptive-rag PULSAR paragraph; pairs K148/K255) + CCC primary (no public code → Watch / 0 MB); 0 SEO Adopt; 0 MB runtime; GW/tipdrop/poker/prod SKIP |
| 2026-08-28 | K165 ingest: 6/6 arXiv API false positives → overflow + CCC thin | RTSE snapshot diagnostics→CCC thin (pairs K282; no code); Bridgeland→overflow only; PPE/Google Earth AI→CCC thin (proprietary no clone); VK-GNN→overflow only (Apache repo OOD no clone); Robbins MDP→overflow only; sepsis clinical→overflow only (wont_wire); 0 SEO Adopt; 0 MB runtime; GW/tipdrop/poker/prod SKIP |
| 2026-08-26 | K164 ingest: 3/3 arXiv API false positives → overflow + thin GEO steal + CCC | Nadiger ion-trap noise→overflow only; Hazra CRASE bounded DeepSearch→thin GEO steal (process-verified-agentic-search-geo) + CCC primary (empty repo → Watch / 0 MB); Corrêa POLAR+PLE VRP→overflow only (MIT repo OOD, no clone); 0 SEO Adopt; 0 MB runtime; GW/tipdrop/poker/prod SKIP |
...
| 2026-08-21 | K162 ingest: 3/3 arXiv API false positives → overflow + CCC primary steal + OSINT thin | Hu CVaR portfolio→OSINT thin (terminal CVaR ⇒ asymmetric state-dependent de-risking; GW SKIP); Kulkarni CLEVER cache eviction→CCC thin (LFU default; quality-adjusted hit 1.1–2.2% vs raw 51–60%; no public GitHub); Fisch Pandora's Router→CCC primary steal (/route VoI; ©2026 Google no clone); 0 SEO Adopt; 0 MB runtime; GW/tipdrop/poker/prod SKIP |
