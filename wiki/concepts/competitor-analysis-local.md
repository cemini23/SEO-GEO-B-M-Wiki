...
Stub concept page for the **methodology** of analyzing local competitors — what to capture, how to capture it, and what to do with the data. Populate via ingest of 2024-2026 local-SEO competitor-analysis frameworks.
...
- Review count + avg rating + recency of reviews + review-text themes
- Photo count + recency of last photo upload
- Posts cadence (recent posts visible on the listing)
- Business hours
- Website URL
- Website platform (inspectable from page source)
- Schema markup present on website (Rich Results Test)
- IG handle + follower count + posting cadence + content style
- TikTok handle (if any)
- Yelp listing
- Facebook Page
- Visible booking-system integration

**Gap analysis**:

Once the per-competitor data is in, the operator's two shops are compared against each. The standard gaps:

1. **Citation gap** — directories where competitors are listed but the operator is not. Tools: BrightLocal, Whitespark, manual scan.
...
4. **GBP-completeness gap** — fields competitors fill that the operator doesn't.
5. **Review-velocity gap** — competitors getting more reviews per month than the operator.
6. **Social-content gap** — content categories competitors are posting that the operator isn't.
